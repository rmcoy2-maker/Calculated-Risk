...
def refresh_button(label: str='🔄 Refresh data', key: str='__refresh__'):
    if not live_enabled():
        st.button(label, key=key, disabled=True)
        st.caption('Beta mode: refresh is disabled (showing cached data).')
        return False
