...
def _settle_minimal(df: pd.DataFrame) -> pd.DataFrame:
    ...
    return d
if not CALCLOG.exists():
    st.info('No Calculated Log found yet.')
    with st.expander('Diagnostics'):
        st.write('Expected path:', str(CALCLOG))
    st.stop()
calcs = pd.read_csv(CALCLOG, encoding='utf-8-sig', low_memory=False)
st.caption(f'Rows: {len(calcs):,}')
need_cols = {'_key_date', '_key_home_sw', '_key_away_sw'}
if need_cols.issubset(set(calcs.columns)):
    ...
    if spath:
        ...
        calcs = _attach_scores_generic(calcs, scores)
        st.caption(f"With scores joined: {int(calcs['_joined_with_scores'].sum()):,} matched")
        calcs = _settle_minimal(calcs)
st.dataframe(cs, width='stretch')
if '_result' in calcs.columns:
    st.subheader('Metrics')
    calcs['market_norm'] = _market_norm(cals.get('market', ''))
    if '_pick_nick' not in calcs.columns or calcs['_pick_nick'].eq('').all():
        calcs['_pick_nick'] = _pick_team_series(calcs)
    team_tbl = calcs.assign(is_win=calcs['_result'].eq('WIN'), is_loss=calcs['_result'].eq('LOSE'), is_push=calcs['_result'].eq('PUSH')).groupby('_pick_nick', dropna=False).agg(calcs=('market_norm', 'size'), wins=('is_win', 'sum'), losses=('is_loss', 'sum'), pushes=('is_push', 'sum'), profit=('_profit_per_$1', 'sum'), roi_per_calc=('_profit_per_$1', 'mean')).reset_index().rename(columns={'_pick_nick': 'team'}).sort_values('profit', ascending=False)
    kind_tbl = calcs.groupby('market_norm', dropna=False).agg(calcs=('market_norm', 'size'), wins=lambda x: (calcs.loc[x.index, '_result'] == 'WIN').sum(), losses=lambda x: (calcs.loc[x.index, '_result'] == 'LOSE').sum(), pushes=lambda x: (calcs.loc[x.index, '_result'] == 'PUSH').sum(), profit=('_profit_per_$1', 'sum'), roi_per_calc=('_profit_per_$1', 'mean')).reset_index().rename(columns={'market_norm': 'market'}).sort_values('profit', ascending=False)
    c1, c2 = st.columns(2)
    with c1:
        st.caption('Team performance (your calcs)')
        st.dataframe(team_tbl, hide_index=True, width='stretch')
        b1 = io.StringIO()
        team_tbl.to_csv(b1, index=False, encoding='utf-8-sig')
        st.download_button('Download team performance CSV', b1.getvalue(), 'calc_history_team_performance.csv', 'text/csv')
    with c2:
        st.caption('calc performance by market')
        st.dataframe(kind_tbl, hide_index=True, width='stretch')
        b2 = io.StringIO()
        kind_tbl.to_csv(b2, index=False, encoding='utf-8-sig')
        st.download_button('Download calc-type breakdown CSV', b2.getvalue(), 'calc_history_calc_types.csv', 'text/csv')
if '_result' in calcs.columns:
    st.subheader('More breakdowns')
    if 'book' in calcs.columns:
        rc = calcs.groupby('book')['_result'].value_counts().unstack(fill_value=0)
        for col in ['WIN', 'LOSE', 'PUSH']:
            if col not in rc.columns:
                rc[col] = 0
        bk = calcs.groupby('book')['_profit_per_$1'].agg(['sum', 'mean'])
        book_tbl = pd.concat([calcs.groupby('book').size().rename('calcs'), rc['WIN'].rename('wins'), rc['LOSE'].rename('losses'), rc['PUSH'].rename('pushes'), bk['sum'].rename('profit'), bk['mean'].rename('roi_per_calc')], axis=1).reset_index().sort_values('profit', ascending=False)
    ...
    calcs['_line_bucket'] = _bucket_series(calcs)
    lb = calcs.groupby('_line_bucket')['_profit_per_$1'].agg(['size', 'sum', 'mean']).reset_index().rename(columns={'size': 'calcs', 'sum': 'profit', 'mean': 'roi_per_calc'}).sort_values('profit', ascending=False)
    st.caption('By line bucket')
    st.dataframe(lb, hide_index=True, width='stretch')
    st.subheader('Cumulative P&L (units)')
    cand_dt = calcs.get('_DateISO', calcs.get('_date_iso', calcs.get('date', calcs.get('commence_time', ''))))
    dt = pd.to_datetime(cand_dt, errors='coerce')
    pnl = calcs.assign(_dt=dt, _p=calcs['_profit_per_$1'].fillna(0)).sort_values('_dt')['_p'].cumsum()
    st.line_chart(pnl, width='stretch')
buf = io.StringIO()
calcs.to_csv(buf, index=False, encoding='utf-8-sig')
st.download_button('Download full Calculated Log CSV', buf.getvalue(), 'calcs_log_full.csv', 'text/csv')
