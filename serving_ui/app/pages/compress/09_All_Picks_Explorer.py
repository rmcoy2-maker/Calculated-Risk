        ...
        for name in ['exports/edges_graded_full.csv', 'exports/edges_master.csv', 'exports/edges_graded.csv', 'exports/edges.csv']:
            if os.path.exists(name) and os.path.getsize(name) > 256:
                ...
                if with_caption:
                    ...
                    st.caption(f'Loaded: {name} · rows={len(df):,} · cols={len(df.columns)} · mtime={mtime:%Y-%m-%d %H:%M:%S}')
    ...
    def autoload_lines(with_caption: bool=False):
        for name in ['exports/lines_live.csv', 'exports/live_lines.csv']:
            if os.path.exists(name) and os.path.getsize(name) > 64:
                ...
                if with_caption:
                    mtime = dt.datetime.fromtimestamp(os.path.getmtime(name))
                    st.caption(f'Live lines: {name} · rows={len(df):,} · cols={len(df.columns)} · mtime={mtime:%Y-%m-%d %H:%M:%S}')
                return (df, name, os.path.abspath('exports'))
        if with_caption:
            st.caption('Live lines: —')
        return (pd.DataFrame(), None, os.path.abspath('exports'))
...
if e['_date'].notna().any():
    st.caption(f"Date range in file: {e['_date'].min():%Y-%m-%d} → {e['_date'].max():%Y-%m-%d}")
L = lines.copy()
house = pd.DataFrame()
if not L.empty:
...
with st.sidebar:
    ...
    date_mode = st.selectbox('Which games?', ['This week (Thu–Mon)', 'Upcoming (>= today)', 'Latest date only', 'Last 3 days', 'All'], index=0)
today = pd.Timestamp.now(tz='UTC').tz_convert(None).normalize()
if date_mode == 'This week (Thu–Mon)':
...
v['✓'] = np.where(like_mask, '✅', '')
if likes_only:
    v = v[like_mask]
BOOK = next((c for c in ['book', 'sportsbook', 'book_name', 'shop'] if c in v.columns), 'book')
cols = [c for c in ['✓', 'game_id', 'market_norm', 'side', 'house_odds', 'house_book', BOOK, 'line', 'odds', 'implied_p(odds)', 'p_model_use', 'p_edge', 'ev/$1', '_date'] if c in v.columns]
st.dataframe(v[cols].reset_index(drop=True), height=560, width='stretch')
st.caption(f'Loaded: {edges_path} · exports root: {root_path} · rows={len(v):,} · cols={len(v.columns)}')
