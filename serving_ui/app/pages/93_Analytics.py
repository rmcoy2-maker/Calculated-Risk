...
# --- Admin/dev override for premium ---
_override_tier = False
try:
    _ss = getattr(st, "session_state", {})
    _role = (_ss.get("user", {}) or {}).get("role") if isinstance(_ss.get("user", {}), dict) else _ss.get("role")
    _plan = (_ss.get("user", {}) or {}).get("plan") if isinstance(_ss.get("user", {}), dict) else _ss.get("plan")
    _override_tier = (_role in {"admin","owner"} or _plan in {"pro","elite","premium"})
except Exception:
    _override_tier = False
try:
    import os as _os
    if str(getattr(st.secrets, "get", lambda *_: False)("DEV_PREMIUM", False)).lower() in {"1","true","yes"}:
        _override_tier = True
    if _os.environ.get("DEV_PREMIUM","") in {"1","true","yes"}:
        _override_tier = True
except Exception:
    pass


# --- Broader admin/dev override ---
_bypass_premium = False
try:
    _qs = getattr(st, "query_params", {})
    _qs_dev = None
    try:
        # Streamlit >= 1.31
        _qs_dev = _qs.get("dev") if hasattr(_qs, "get") else None
    except Exception:
        _qs_dev = None
    _qs_flag = (str(_qs_dev) if _qs_dev is not None else "").lower() in {"1","true","yes"}
    _ss = getattr(st, "session_state", {})
    _is_admin = any([
        bool(_ss.get("is_admin")),
        str(_ss.get("role","")).lower() in {"admin","owner"},
        str((_ss.get("user",{}) or {}).get("role","")).lower() in {"admin","owner"},
    ])
    _is_premium_plan = str(_ss.get("plan","")).lower() in {"pro","elite","premium"} or \
                       str((_ss.get("user",{}) or {}).get("plan","")).lower() in {"pro","elite","premium"}
    _bypass_premium = _qs_flag or _is_admin or _is_premium_plan
except Exception:
    pass
try:
    import os as _os
    if str(getattr(st.secrets, "get", lambda *_: False)("DEV_PREMIUM", False)).lower() in {"1","true","yes"}:
        _bypass_premium = True
    if str(getattr(st.secrets, "get", lambda *_: False)("ADMIN_OVERRIDE", False)).lower() in {"1","true","yes"}:
        _bypass_premium = True
    if _os.environ.get("DEV_PREMIUM","").lower() in {"1","true","yes"}:
        _bypass_premium = True
except Exception:
    pass
# --- Premium gate (soft) ---
if not (premium_enabled() or _override_tier or _bypass_premium):
    st.warning("Premium-only page — limited mode. (Admin/dev can add ?dev=1 or set DEV_PREMIUM=1 to unlock)")
    # No st.stop(): allow view in limited mode
# === Repo roots & paths ===
