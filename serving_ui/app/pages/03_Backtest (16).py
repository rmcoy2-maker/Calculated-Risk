...
def _fix_side_labels(df):
    """
    Normalize the 'side' column to one of {'home','away','over','under'} if present.
    Leaves rows untouched if column missing.
    """
    import pandas as _pd
    if df is None or not isinstance(df, _pd.DataFrame) or "side" not in df.columns:
        return df
    mapping = {
        "h":"home","home":"home","host":"home",
        "a":"away","away":"away","visitor":"away","vis":"away",
        "o":"over","over":"over",
        "u":"under","under":"under"
    }
    def norm(x):
