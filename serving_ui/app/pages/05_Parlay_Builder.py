...
# === UltraImportGuard (parlay_ui) — registers app.utils.parlay_ui even if packages not set ===
try:
    from app.utils.parlay_ui import selectable_odds_table  # normal path
except Exception:
    import sys, types, importlib.util
    from pathlib import Path
    here = Path(__file__).resolve()
...
try:
    if live_enabled():
        try:
            from app.lib.access import do_expensive_refresh  # optional
        except Exception:
...
display_cols = [c for c in ['_date_iso', 'home', 'away', '_home_nick', '_away_nick', '_market_norm', 'side', 'line', 'price', 'p_win', '_ev_per_$1'] if c in pool.columns]
...
# Optional: allow adding live items via table
try:
