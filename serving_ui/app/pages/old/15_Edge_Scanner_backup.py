...
def find_scanner() -> Path | None:
    cands = [REPO / 'tools' / 'scan_edges.py', REPO / 'serving' / 'scan_edges.py', REPO / 'serving_ui' / 'scan_edges.py']
    for p in cands:
        if p.exists():
            return p
    return None
...
def read_edges() -> tuple[pd.DataFrame, Path | None]:
    cands = [EXPORTS / 'edges.csv', EXPORTS / 'edges_graded_wlp_clean.csv', EXPORTS / 'edges_graded_wlp.csv', EXPORTS / 'edges_graded.csv']
...
def sample_edges_file():
    ...
    data = [{'game_id': 'Arizona Cardinals@San Francisco 49ers_2025-09-21T20:26:00Z', 'market': 'moneyline', 'side': 'ARIZONA', 'book': 'sample', 'odds': +245, 'p_win': 0.29}, {'game_id': 'Dallas Cowboys@Chicago Bears_2025-09-21T20:25:00Z', 'market': 'moneyline', 'side': 'CHICAGO', 'book': 'sample', 'odds': -115, 'p_win': 0.53}]
...
if btn:
    if not scanner_path:
        ...
        with st.status(f'Running scanner: {scanner_path.name}', expanded=True) as status:
            try:
                proc = subprocess.run([sys.executable, str(scanner_path)], cwd=str(scanner_path.parent), capture_output=True, text=True, timeout=180)
...
st.dataframe(edges_view[cols].reset_index(drop=True), height=520, width='stretch')
st.caption(f'Rows after filters: {len(edges_view):,}')
