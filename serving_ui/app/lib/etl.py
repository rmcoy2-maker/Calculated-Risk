...
def ev_per_dollar(odds_american, p):
    if pd.isna(odds_american) or pd.isna(p):
        return np.nan
...
def normalize_edges(df: pd.DataFrame) -> tuple[pd.DataFrame, dict]:
    ...
    if df is None or df.empty:
        return (df.copy(), {'rows_total': 0, 'rows_with_odds': 0, 'rows_with_p': 0, 'rows_pos_ev': 0})
    w = df.copy()
    ...
    diag = {'rows_total': len(w), 'rows_with_odds': int(w['odds_std'].notna().sum()), 'rows_with_p': int(pd.to_numeric(w['p_win_std'], errors='coerce').notna().sum()), 'rows_pos_ev': int((pd.to_numeric(w['ev_std'], errors='coerce') >= 0).sum())}
    return (w, diag)