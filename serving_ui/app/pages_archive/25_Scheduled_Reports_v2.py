...
if 'model_version' in settled.columns:
    by_modelver = settled.groupby('model_version', dropna=True).agg(bets=('stake', 'count'), stake=('stake', _num), profit=('payout', _num)).reset_index()
    by_modelver['roi_%'] = by_modelver['profit'] / by_modelver['stake'] * 100.0
    tmp = settled.copy()
    if 'ts' in tmp.columns:
        tmp['ts'] = pd.to_datetime(tmp['ts'], errors='coerce', utc=True)
        tmp['week'] = tmp['ts'].dt.to_period('W-MON').dt.start_time
    else:
        tmp['week'] = 'n/a'
    by_week_modelver = tmp.groupby(['week', 'model_version']).agg(bets=('stake', 'count'), stake=('stake', _num), profit=('payout', _num)).reset_index()
    by_week_modelver['roi_%'] = by_week_modelver['profit'] / by_week_modelver['stake'] * 100.0
else:
    by_modelver = pd.DataFrame()
    by_week_modelver = pd.DataFrame()
mem = io.BytesIO()
with zipfile.ZipFile(mem, mode='w', compression=zipfile.ZIP_DEFLATED) as z:
    ...
    z.writestr('by_week_modelver.csv', by_week_modelver.to_csv(index=False))
