...
st.caption('Evaluate probability quality using **settled bets** from your Calculated Log (Brier, LogLoss, reliability curve).')
df = read_log()
if df.empty:
...
ll = log_loss(y, p)
c1, c2 = st.columns(2)
c1.metric('Brier Score (Ã¢â€\xa0â€œ better)', f'{brier:.4f}')
c2.metric('Log Loss (Ã¢â€\xa0â€œ better)', f'{ll:.4f}')
...
st.caption('A perfectly calibrated model lies close to the diagonal (y_rate Ã¢â€°Ë† p_avg).')
with st.expander('Calibration table'):
    st.dataframe(cal, hide_index=True, width='stretch')
    st.download_button('Download calibration.csv', data=cal.to_csv(index=False).encode(), file_name='calibration.csv', mime='text/csv')
