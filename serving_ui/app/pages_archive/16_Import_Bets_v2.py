...
st.caption('Upload a sportsbook/tracker CSV, map columns to your schema, and export a standardized file you can append to your Calculated Log.')
STANDARD_FIELDS: List[str] = ['ts', 'game_id', 'market', 'ref', 'side', 'selection', 'league', 'sport', 'book', 'line', 'odds', 'p_win', 'stake', 'result', 'payout', 'open_odds', 'close_odds', 'season', 'week']
    ...
    def has(*keys):
        for k in keys:
            if k in cset:
                return cset[k]
        return '(skip)'
    m = {'ts': has('ts', 'timestamp', 'date', 'placed_at'), 'game_id': has('game_id', 'gid', 'event_id', 'match_id'), 'market': has('market', 'bet_type', 'wager_type'), 'ref': has('ref', 'id', 'bet_id', 'wager_id'), 'side': has('side', 'pick', 'bet_on', 'outcome'), 'selection': has('selection', 'team', 'player', 'runner'), 'league': has('league', 'lg'), 'sport': has('sport'), 'book': has('book', 'sportsbook', 'source'), 'line': has('line', 'number', 'spread', 'total'), 'odds': has('odds', 'price', 'american_odds'), 'p_win': has('p_win', 'prob', 'win_prob', 'model_p'), 'stake': has('stake', 'risk', 'wager', 'amount'), 'result': has('result', 'status', 'settlement'), 'payout': has('payout', 'return', 'to_win', 'win_amount'), 'open_odds': has('open_odds', 'orig_odds', 'opening_odds'), 'close_odds': has('close_odds', 'closing_odds', 'close'), 'season': has('season', 'yr', 'year'), 'week': has('week', 'wk')}
    return m
...
for i, field in enumerate(STANDARD_FIELDS):
    with grid_cols[i % 3]:
        ...
        mapping[field] = st.selectbox(f'Map â†’ **{field}**', options=cols, index=default_idx, key=f'map_{field}')
...
if st.button('Build standardized dataset'):
    ...
    st.download_button('Download standardized_bets.csv', data=df.to_csv(index=False).encode(), file_name='standardized_bets.csv', mime='text/csv')
    with st.expander('Sample (first 30 rows)'):
        st.dataframe(df.head(30), hide_index=True, width='stretch')
st.subheader('Append to Calculated Log (Optional)')
...
if st.button('Append now', disabled=not append_ok):
    ...
    with st.expander('Log tail (last 30 rows)'):
        st.dataframe(merged.tail(30), hide_index=True, width='stretch')
