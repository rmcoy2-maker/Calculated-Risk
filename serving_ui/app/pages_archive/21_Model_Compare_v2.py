...
for name, p in [('A', pa), ('B', pb)]:
    metrics.append({'model': name, 'brier': brier_score(y, p), 'log_loss': log_loss(y, p)})
...
st.dataframe(d_view[cols_show + ['pa', 'pb', 'abs_diff']].head(50), hide_index=True, width='stretch')
st.download_button('Download disagreements.csv', data=d_view[cols_show + ['pa', 'pb', 'abs_diff']].to_csv(index=False).encode(), file_name='ab_disagreements.csv', mime='text/csv')