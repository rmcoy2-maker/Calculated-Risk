...
if st.button('âž• Add to Calculated Log (with model_version)'):
    rec = {'ts': None, 'model_version': MODEL_VERSION, 'game_id': str(row['game_id']) if 'game_id' in row.index and pd.notna(row['game_id']) else '', 'market': str(row['market']) if 'market' in row.index and pd.notna(row['market']) else '', 'ref': ref, 'side': side, 'selection': selection, 'league': str(row['league']) if 'league' in row.index and pd.notna(row['league']) else '', 'sport': str(row['sport']) if 'sport' in row.index and pd.notna(row['sport']) else '', 'book': book, 'line': line, 'odds': odds, 'p_win': p_win, 'stake': stake, 'result': result, 'payout': None, 'open_odds': odds, 'close_odds': None, 'season': int(row['season']) if 'season' in row.index and str(row['season']).isdigit() else None, 'week': int(row['week']) if 'week' in row.index and str(row['week']).isdigit() else None}
