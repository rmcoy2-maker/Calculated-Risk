...
def text_input_once(label, keyname, default='', **kw):
    uk = k(keyname)
    if st.session_state.get(_created_flag(uk)):
        return st.session_state.get(uk, default)
    v = st.text_input(label, value=st.session_state.get(uk, default), key=uk, **kw)
    st.session_state[_created_flag(uk)] = True
    return v
...
def checkbox_once(label, keyname, default=False, **kw):
    uk = k(keyname)
    if st.session_state.get(_created_flag(uk)):
        return bool(st.session_state.get(uk, default))
...
def date_input_once(label, keyname, **kw):
    uk = k(keyname)
    if st.session_state.get(_created_flag(uk)):
        return st.session_state.get(uk)
    v = st.date_input(label, key=uk, **kw)
    st.session_state[_created_flag(uk)] = True
    return v
FIXED_NAMES = ['schedule_2025_template.csv', 'games_2025_more.csv', 'market_lines.csv', 'odds_books.csv', 'lines.csv']
GLOBS = ['market_lines_*.csv', 'schedule_*.csv', 'games_*_more.csv']
...
def _parse_season_token(s: str) -> Tuple[Optional[str], Optional[int], Optional[int]]:
    ...
    s = str(s).strip()
    m = re.search('(\\d{4})\\s*[-/Ã¢â‚¬â€œ]\\s*(\\d{2,4})', s)
    if m:
        a = int(m.group(1))
        b_raw = m.group(2)
...
def pick_best_source(paths: Iterable[Path]) -> Tuple[Optional[Path], Optional[str], Optional[int]]:
    ...
    for p in paths:
        ...
        if season_end is not None:
            key = (1, season_end, season_start or 0, mtime)
        else:
            key = (0, 0, 0, mtime)
...
if LINES_PATH is None:
    st.error(f'Ã¢Â\x9dÅ’ No valid lines/schedule file found in {DB_DIR}/')
...
def normalize_market(s: str) -> str:
    s = str(s).strip().lower()
    return {'h2h': 'H2H', 'moneyline': 'H2H', 'ml': 'H2H', 'mline': 'H2H', 'spread': 'SPREADS', 'spreads': 'SPREADS', 'handicap': 'SPREADS', 'total': 'TOTALS', 'totals': 'TOTALS', 'o/u': 'TOTALS', 'ou': 'TOTALS'}.get(s, s.upper())
...
def normalize_lines(df: pd.DataFrame) -> pd.DataFrame:
    ...
    if 'side' in out.columns:
        out['side'] = out['side'].astype(str).str.upper().replace({'HOME': 'HOME', 'H': 'HOME', 'AWAY': 'AWAY', 'A': 'AWAY'})
...
if has_season:
    season_table = df[['__season_label', '__season_end']].dropna().drop_duplicates().sort_values(['__season_end', '__season_label'])
...
with st.sidebar:
    ...
    if not teams:
        ...
        if 'home_team' in df.columns:
            ha += _unique_str(df, 'home_team')
        if 'away_team' in df.columns:
            ha += _unique_str(df, 'away_team')
        teams = sorted(set(ha))
...
if team_search:
    ...
    if terms:
        cols = []
        if 'team' in view.columns:
            cols.append(view['team'].astype(str).str.lower())
        if 'home_team' in view.columns:
            cols.append(view['home_team'].astype(str).str.lower())
        if 'away_team' in view.columns:
            cols.append(view['away_team'].astype(str).str.lower())
        if cols:
            mask_any = False
            for s in cols:
                cm = False
...
cols_all = ['event_time', '__season_label', 'season', 'week', 'market', 'game_id', 'home_team', 'away_team', 'side', 'book', 'line', 'odds', 'dec_odds', 'imp']
cols_show = [c for c in cols_all if c in view.columns]
st.subheader('All books')
st.dataframe(view[cols_show] if cols_show else view, width='stretch')
st.download_button('Ã¢Â¬â€¡Ã¯Â¸Â\x8f Download filtered lines.csv', data=view.to_csv(index=False).encode('utf-8'), file_name='lines_filtered.csv', mime='text/csv')
with st.expander('Ã°Å¸Â\x8fâ€\xa0 Best price per game & side'):
    if 'dec_odds' not in view.columns:
        ...
        if not group_keys:
            ...
            cols_best = [c for c in ['event_time', '__season_label', 'season', 'week', 'market', 'game_id', 'side', 'best_book', 'best_odds', 'best_dec', 'best_imp', 'hnew_pct'] if c in best.columns]
            st.dataframe(best[cols_best] if cols_best else best, width='stretch')
with st.expander('Ã¢Å¡â„¢Ã¯Â¸Â\x8f Diagnostics'):
    st.write('REPO:', REPO)
    ...
    try:
        ...
        st.dataframe(pd.read_csv(LINES_PATH, nrows=8), width='stretch')
