...
def ghost_optimize(legs: List[Leg]) -> Tuple[List[Leg], Dict[str, Any]]:
    ...
    while improved and len(current) >= 2:
        ...
        if best_idx is not None:
            ...
            drops.append({'removed': removed.label, 'gain': best_gain, 'new_ev': best_ev, 'new_dec': best_dec})
            ev_now = best_ev
            improved = True
...
def make_legs(df: pd.DataFrame) -> List[Leg]:
    ...
    for _, r in df.iterrows():
        try:
            legs.append(Leg(game_id=str(r.get('game_id', '')), market=str(r.get('market', '')), side=str(r.get('side', '')), book=str(r.get('book', '')), odds=int(r.get('odds', 0)), p_win=float(r.get('p_win', 0.0))))
...
def filter_pool(legs: List[Leg], *, min_p: float, min_abs_odds: int, enforce_distinct_games: bool) -> List[Leg]:
    pool = [lg for lg in legs if (lg.p_win is not None and float(lg.p_win) > 0) and lg.p_win >= min_p and (abs(int(lg.odds)) >= min_abs_odds or True)]
    ...
    for r in range(2, max(2, max_legs) + 1):
        for combo in combinations(pool, r):
            ...
            if limit_combos and count >= limit_combos:
                return cands
    return cands
st.set_page_config(page_title='ðŸ‘» Ghost Parlay Calculator', layout='wide')
st.title('ðŸ‘» Ghost Parlay Calculator')
ENV_ROOT = os.getenv('EDGE_FINDER_ROOT', '')
ROOT = Path(ENV_ROOT) if ENV_ROOT else _REPO_ROOT
EXPORT = ROOT / 'exports'
EDGES = EXPORT / 'edges.csv'
CANDS = EXPORT / 'hedge_candidates.csv'
...
with st.sidebar:
    ...
    min_p = st.slider('Min leg win prob (p_win)', 0.4, 0.8, 0.5, 0.01)
    ...
    min_roi = st.number_input('Min ROI (per unit)', value=0.01, step=0.01, format='%.2f')
    min_kelly = st.number_input('Min Kelly fraction', value=0.0, step=0.01, format='%.2f')
...
def legs_to_json(legs: List[Leg]) -> str:
    return json.dumps([{'game_id': lg.game_id, 'market': lg.market, 'side': lg.side, 'book': lg.book, 'odds': lg.odds, 'p_win': lg.p_win} for lg in legs])
if run_build:
    with st.spinner('Building combos and ghost-optimizing...'):
        ...
        for combo in combos:
            ...
            if roi1 >= min_roi and kf >= min_kelly:
                results_rows.append({'legs': ' + '.join((l.label for l in opt_legs)), 'n_legs': len(opt_legs), 'american': decimal_to_american(d1), 'decimal': round(d1, 4), 'p_combo': round(combo_prob(opt_legs), 6), 'ev_unit': round(ev1, 6), 'roi': round(roi1, 6), 'kelly': round(kf, 6), 'drops': '; '.join((f"[-]{d['removed']} (Î”EV={d['gain']:+.4f})" for d in details.get('drops', []))), 'legs_json': legs_to_json(opt_legs)})
    if not results_rows:
        st.warning('No candidates met the threshnews. Try lowering Min ROI / increasing pool.')
    else:
        res_df = pd.DataFrame(results_rows)
        res_df.sort_values(['roi', 'kelly', 'american'], ascending=[False, False, False], inplace=True)
        st.subheader('Top Ghosted Parlays')
        st.dataframe(res_df.head(show_n), width='stretch')
        ...
        with c2:
            if st.button('Append top N to bets_log.csv (as suggestions)'):
                ...
                for _, r in take.iterrows():
                    rows.append({'ts': datetime.utcnow().isoformat(), 'game_id': 'PARLAY', 'market': 'GHOST', 'ref': r.get('legs', ''), 'side': '-', 'line': 0, 'odds': int(r.get('american', 0)), 'p_win': float(r.get('p_combo', 0.0)), 'ev': float(r.get('ev_unit', 0.0))})
...
with st.expander('Preview source edges (first 500 rows)'):
    st.dataframe(edges_df.head(500), width='stretch')
