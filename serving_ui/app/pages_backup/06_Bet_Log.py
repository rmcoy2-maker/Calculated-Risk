...
try:
    ...
    def ensure_bet_log() -> Path:
        ...
        if not p.exists():
            pd.DataFrame(columns=['ts', 'market', 'ref', 'side', 'line', 'odds', 'p_win', 'ev', 'stake', 'status', 'result', 'pnl', 'tag', 'legs_json', 'bet_id']).to_csv(p, index=False, encoding='utf-8')
        return p
    ...
    def legs_short_from_json(s):
        try:
            ...
            if isinstance(legs, dict):
                legs = legs.get('legs', [])
            return ' + '.join((f"{str(d.get('side', '?'))}@{str(d.get('odds', '?'))}" for d in legs)) or '(no legs)'
        except Exception:
            return '(invalid legs)'
...
def read_parlays() -> pd.DataFrame:
    if not PARLAYS.exists():
        return pd.DataFrame(columns=['ts', 'market', 'odds', 'p_win', 'ev', 'stake', 'status', 'result', 'pnl', 'tag', 'legs_json', 'parlay_id'])
...
st.set_page_config(page_title='Calculated Log', layout='wide')
st.title('ðŸ“’ Calculated Log')

def _norm(series):
    return pd.Series(series, dtype='object').astype(str).str.lower().str.strip().replace({'': None})
...
expected = ['ts', 'market', 'ref', 'side', 'line', 'odds', 'p_win', 'ev', 'stake', 'status', 'result', 'pnl', 'tag', 'legs_json', 'bet_id', 'origin']
for c in expected:
...
loss = int((res == 'loss').sum())
push = int((res == 'push').sum())
...
by_origin = df_perf.groupby('origin', dropna=False).agg(Bets=('ref', 'count'), Staked=('stake', 'sum'), PnL=('pnl', 'sum'))
with np.errstate(divide='ignore', invalid='ignore'):
    by_origin['ROI%'] = by_origin['PnL'] / by_origin['Staked'].replace(0, np.nan) * 100
st.dataframe(by_origin.fillna(0).round(2), width='stretch')
    ...
    if not HAS_MPL:
        ...
        if 'stake' in df_filt.columns and (not df_filt.empty):
            ...
            if not _stake.empty:
                ...
                plt.xlabel('Stake')
                plt.ylabel('Count')
                plt.title('Stake size distribution')
                st.pyplot(fig)
        if {'ev', 'result'}.issubset(df_perf.columns) and (not df_perf.empty):
            ...
            if non_nan.sum() >= 5:
                try:
                    ...
                    wr.plot(kind='bar')
                    plt.ylabel('Win %')
                    plt.title('Win% by EV bucket (closed, pushes ignored)')
                    st.pyplot(fig)
                except Exception:
...
if df_perf['ts'].notna().any():
    ...
    weekly = temp.set_index('ts').resample('W').agg(staked=('stake_num', 'sum'), pnl=('pnl_num', 'sum'))
    if not weekly.empty:
        with np.errstate(divide='ignore', invalid='ignore'):
...
if df_perf['ts'].notna().any() and 'origin' in df_perf.columns:
    ...
    temp['stake_num'] = _num(temp.get('stake')).fillna(0)
    temp['pnl_num'] = _num(temp.get('pnl')).fillna(0)
    weekly = temp.set_index('ts').groupby('origin').resample('W').agg(staked=('stake_num', 'sum'), pnl=('pnl_num', 'sum')).reset_index()
    if not weekly.empty:
        with np.errstate(divide='ignore', invalid='ignore'):
            weekly['ROI%'] = weekly['pnl'] / weekly['staked'].replace(0, np.nan) * 100
        if not weekly['ROI%'].dropna().empty:
...
st.dataframe(open_df[['bet_id', 'market', 'ref', 'legs_short', 'odds', 'stake', 'status', 'tag']], hide_index=True, width='stretch')
st.subheader('Closed / Settled')
closed_like = {'closed', 'settled', 'graded', 'void', 'canceled', 'cancelled'}
result_like = {'win', 'loss', 'push', 'void'}
closed_df = df_filt[df_filt['status'].isin(closed_like) | df_filt['result'].isin(result_like)]
st.dataframe(closed_df[['bet_id', 'ts', 'market', 'ref', 'legs_short', 'odds', 'stake', 'status', 'result', 'pnl', 'tag']], hide_index=True, width='stretch')
st.subheader('All Entries')
st.dataframe(df_filt[['bet_id', 'ts', 'market', 'ref', 'legs_short', 'odds', 'p_win', 'ev', 'stake', 'status', 'result', 'pnl', 'tag']], hide_index=True, width='stretch')
csv_bytes = df_filt.to_csv(index=False, encoding='utf-8').encode('utf-8')
st.download_button('â¬‡ï¸\x8f Download filtered bets.csv', data=csv_bytes, file_name='filtered_bets.csv', mime='text/csv')
if st.button('ðŸ§¹ Normalize & Save bets_log.csv'):
    df.to_csv(log_path, index=False, encoding='utf-8')
    st.success(f'Normalized and saved â†’ {log_path}')
    if hasattr(st, 'rerun'):
        st.rerun()
    else:
        st.rerun()
