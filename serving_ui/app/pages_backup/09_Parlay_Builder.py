...
def log_parlay(legs, odds, p_win, ev, stake=None, tag='manual', notes=''):
    row = {'ts': utc_ts(), 'market': 'parlay', 'odds': odds, 'p_win': p_win, 'ev': ev, 'stake': stake, 'status': 'open', 'result': None, 'pnl': None, 'tag': tag, 'legs_json': json.dumps(legs), 'parlay_id': uuid.uuid4().hex[:12], 'notes': notes}
    append_dict_row(PARLAYS_CSV, row, PARLAY_HEADER)
    return row

def log_bet(legs, odds, p_win, ev, stake=None, tag='manual', notes=''):
    row = {'ts': utc_ts(), 'market': 'parlay', 'ref': None, 'side': None, 'line': None, 'odds': odds, 'p_win': p_win, 'ev': ev, 'stake': stake, 'status': 'open', 'result': None, 'pnl': None, 'tag': tag, 'legs_json': json.dumps(legs), 'bet_id': uuid.uuid4().hex[:12], 'notes': notes}
...
with st.expander('Manual Build', expanded=True):
    ...
    st.dataframe(pd.DataFrame(legs), hide_index=True, width='stretch')
    if legs:
        ...
        with k1:
            bankroll_val = st.number_input('Bankroll (from ledger)', min_value=0.0, step=10.0, value=float(bk_default))
        with k2:
            kelly_scale = st.selectbox('Kelly scale', ['1.00 (Full)', '0.50 (Half)', '0.25 (Quarter)'], index=1)
        with k3:
            k_mult = {'1.00 (Full)': 1.0, '0.50 (Half)': 0.5, '0.25 (Quarter)': 0.25}[kelly_scale]
        k_frac = kelly_fraction(p, am, k_mult)
        k_stake = round(bankroll_val * k_frac, 2) if bankroll_val and k_frac else 0.0
        s1, s2 = st.columns([1, 2])
        with s1:
            st.metric('Kelly fraction', f'{k_frac:.2%}')
        with s2:
            stake_input = st.number_input('Stake (defaults to Kelly recommendation)', min_value=0.0, step=1.0, value=float(k_stake))
        notes = st.text_input('Notes', key='manual_notes')

        def _bucket_market(m: str) -> str:
            s = str(m or '').lower()
            if 'moneyline' in s or s in ('ml', 'h2h'):
                return 'Moneyline'
            if 'spread' in s:
                return 'Spread'
            if 'total' in s:
        ...
        with st.expander('Summary', expanded=True):
            ...
            if not _df.empty:
                ...
                book_counts = _df['book'].fillna('â€”').value_counts().rename_axis('book').reset_index(name='count') if 'book' in _df.columns else pd.DataFrame({'book': [], 'count': []})
                sA, sB, sC = st.columns(3)
                with sA:
                    st.caption('Legs by type')
                    st.dataframe(type_counts, hide_index=True, width='stretch')
                with sB:
                    st.caption('Legs by book')
                    st.dataframe(book_counts, hide_index=True, width='stretch')
                with sC:
        ...
        a, b = st.columns(2)
        with a:
            if st.button('ðŸ“\x9d Log Manual Parlay'):
                try:
        ...
        with b:
            if st.button('ðŸ—‘ï¸\x8f Clear Legs'):
                st.session_state['picked_legs'] = []
                st.rerun()
with st.expander('Pick legs (edges.csv) + Build from market_lines', expanded=False):
    ...
    def _build_edges_from_lines(df_src: pd.DataFrame) -> pd.DataFrame:
        ...
        ren = {'sportsbook': 'book', 'book_name': 'book', 'bookmaker': 'book', 'site': 'book', 'american': 'odds', 'price': 'odds', 'bet_type': 'market', 'market_type': 'market', 'wager_type': 'market', 'selection': 'side', 'team': 'side', 'total': 'line', 'points': 'line', 'threshnew': 'line', 'prob': 'p_win', 'probability': 'p_win', 'implied_prob': 'p_win', 'note': 'ref', 'id': 'ref'}
        ...
        def mk_game_id(r):
            ...
            return None
        rows = []
        ml_mask = d['market'].astype(str).str.contains('\\b(ml|moneyline|h2h)\\b', case=False, na=False)
        for _, r in d[ml_mask].iterrows():
            rows.append({'game_id': mk_game_id(r), 'market': 'Moneyline', 'side': r.get('side'), 'line': None, 'book': r.get('book'), 'odds': _to_int_odds(r.get('odds')), 'p_win': r.get('p_win'), 'ref': r.get('ref')})
        sp = d[d['market'].astype(str).str.contains('spread', case=False, na=False)]
        for _, r in sp.iterrows():
            rows.append({'game_id': mk_game_id(r), 'market': 'Spread', 'side': r.get('side'), 'line': r.get('line'), 'book': r.get('book'), 'odds': _to_int_odds(r.get('odds')), 'p_win': r.get('p_win'), 'ref': r.get('ref')})
        ...
        for _, r in tot.iterrows():
            ...
            rows.append({'game_id': mk_game_id(r), 'market': 'Total', 'side': ou, 'line': r.get('line'), 'book': r.get('book'), 'odds': _to_int_odds(r.get('odds')), 'p_win': r.get('p_win'), 'ref': r.get('ref')})
        ...
        for _, r in pr.iterrows():
            ...
            rows.append({'game_id': mk_game_id(r), 'market': f'Prop: {ptype}', 'side': side, 'line': r.get('line'), 'book': r.get('book'), 'odds': _to_int_odds(r.get('odds')), 'p_win': r.get('p_win'), 'ref': r.get('ref')})
    ...
    if edges_df is None or edges_df.empty:
        ...
        with r1:
            bucket_choice = st.radio('Bet type', ['All', 'Moneyline', 'Spread', 'Total (O/U)', 'Props'], horizontal=True, key='picker_bucket_choice')
        with r2:
            txt_search = st.text_input('Search team / player / text', key='picker_search', placehnewer='e.g., Dolphins, Over, Mahomes, -3.5')
        ...
        st.dataframe(disp_filt.head(200), hide_index=True, width='stretch')
        work = disp_filt.copy()
        work.insert(0, 'add', False)
        edited = st.data_editor(work, hide_index=True, num_rows='dynamic', key='edges_editor', width='stretch')
        cA, cB, cC = st.columns([1, 1, 1])
        with cA:
            if st.button('âž• Add selected rows to Parlay'):
                ...
                if add_rows.empty:
                    ...
                    def row_key(r):
                        return (str(r.get('game_id')), str(r.get('market')), str(r.get('side')), str(r.get('line')), str(r.get('book')), str(r.get('odds')))
                    base = edges_df.copy()
                    ...
                    for _, r in add_rows.iterrows():
                        ...
                        if not match.empty:
                            ...
                            to_add.append({'game_id': full.get('game_id'), 'market': full.get('market'), 'side': full.get('side'), 'line': full.get('line'), 'book': full.get('book'), 'odds': int(str(full.get('odds')).replace('+', '')) if pd.notna(full.get('odds')) else None, 'p_win': full.get('p_win'), 'ref': full.get('ref') if pd.notna(full.get('ref')) else uuid.uuid4().hex[:8]})
                    cur = st.session_state.get('picked_legs', [])
                    exists = set(((str(x.get('game_id')), str(x.get('market')), str(x.get('side')), str(x.get('line')), str(x.get('book')), str(x.get('odds'))) for x in cur))
                    for leg in to_add:
                        k = (str(leg.get('game_id')), str(leg.get('market')), str(leg.get('side')), str(leg.get('line')), str(leg.get('book')), str(leg.get('odds')))
                        if k not in exists:
                            cur.append(leg)
                            exists.add(k)
                    ...
                    st.rerun()
        with cB:
            if st.button('ðŸ§¹ Deduplicate current legs'):
                cur = st.session_state.get('picked_legs', [])
                seen, out = (set(), [])
                for leg in cur:
                    k = (str(leg.get('game_id')), str(leg.get('market')), str(leg.get('side')), str(leg.get('line')), str(leg.get('book')), str(leg.get('odds')))
                ...
                st.success(f'Deduped â†’ {len(out)} unique legs.')
                st.rerun()
        with cC:
            if st.button('â†©ï¸\x8f Remove last leg'):
                cur = st.session_state.get('picked_legs', [])
                if cur:
                    cur.pop()
                    st.session_state['picked_legs'] = cur
...
with st.expander('Add a custom leg', expanded=False):
    ...
    if mkt_choice == 'Moneyline':
        ...
        if st.button('âž• Add Moneyline leg'):
        ...
        with cs1:
            side_team = st.text_input('Team / Side (e.g., Dolphins)', '')
        with cs2:
            line_txt = st.text_input('Spread (e.g., -3.5)', '')
        if st.button('âž• Add Spread leg'):
        ...
        with ct1:
            ou = st.selectbox('O/U', ['Over', 'Under'], index=0)
        with ct2:
            total_line = st.number_input('Total line', step=0.5, value=45.5)
        if st.button('âž• Add Total (O/U) leg'):
        ...
        with cp2:
            stat = st.selectbox('Stat', ['Points', 'Rebounds', 'Assists', 'Yards', 'Rush Yds', 'Rec Yds', 'Pass Yds', 'Other'], index=0)
        with cp3:
            ou = st.selectbox('O/U', ['Over', 'Under'], index=0)
        with cp4:
            prop_line = st.number_input('Line', step=0.5, value=20.5)
        if st.button('âž• Add Prop leg'):
    ...
    if leg is not None:
        ...
        cur.append(leg)
        st.session_state['picked_legs'] = cur
        st.success(f"{leg['market']} leg added.")
        st.rerun()
with st.expander('Validate current parlay (correlation guard)', expanded=False):
    ...
    if not legs:
        ...
        if ok:
            st.success('âœ… Correlation OK')
        else:
            st.error(f'â\x9dŒ Correlation guard blocked this parlay:\n\n{msg}')
with st.expander('ðŸ”\x81 Refresh edges', expanded=False):
    c1, c2 = st.columns(2)
    with c1:
    ...
    with c2:
        if st.button('Rebuild from market_lines â†’ edges.csv', key='rebuild_from_lines'):
            try:
                lines_2025 = EXPORTS / 'market_lines_2025.csv'
                lines_any = EXPORTS / 'market_lines.csv'
                ...
                if src_path is None:
                    ...
                    if new_edges is None or new_edges.empty:
                        ...
                        st.success(f'Wrote {len(new_edges)} rows â†’ {outp.name}')
                        st.rerun()
            except Exception as e:
                st.error(f'Rebuild failed: {e}')
st.subheader('ðŸ¤– Computer Suggested Parlays')