...
def settle_row(row) -> tuple[float, bool]:
    try:
        ...
        if res in ('win', 'won', 'w'):
            return (stake * (dec - 1.0), True)
        if res in ('lose', 'lost', 'l'):
            return (-stake, True)
        if res in ('push', 'void', 'cancel', 'canceled', 'cancelled'):
            return (0.0, True)
        return (0.0, False)
    except Exception:
        return (0.0, False)
...
def normalize_bets(df: pd.DataFrame, source_tag: str) -> pd.DataFrame:
    ...
    df['src'] = source_tag
    return df
_MARKET_GROUPS = {'Moneyline': ['ml', 'moneyline', 'money line', 'money-line'], 'Spread': ['spread', 'ats', 'handicap', 'point spread'], 'Total (O/U)': ['total', 'totals', 'o/u', 'over/under', 'ou'], 'Props': ['prop', 'props', 'player', 'passing', 'rushing', 'receiving', 'td', 'interception', 'tackles', 'sacks'], 'Parlay': ['parlay']}
...
def summarize(df: pd.DataFrame, curve: pd.DataFrame) -> pd.DataFrame:
    ...
    realized_mask = df.get('result', pd.Series(dtype=str)).astype(str).str.lower().isin(['win', 'won', 'w', 'lose', 'lost', 'l', 'push', 'void', 'cancel', 'canceled', 'cancelled'])
    ...
    return pd.DataFrame([{'bets': len(df), 'settled': int(settled), 'wins': int(wins), 'losses': int(losses), 'pushes': int(pushes), 'staked_units': round(total_staked, 2), 'realized_pnl': round(realized_pnl, 2), 'roi': round(roi, 4) if pd.notna(roi) else np.nan, 'avg_odds': round(float(avg_odds), 2) if pd.notna(avg_odds) else np.nan, 'max_drawdown': round(max_dd, 2)}])
...
def apply_policy(df: pd.DataFrame, policy: str, bankroll_start: float, unit_size: float, kelly_frac: float) -> pd.DataFrame:
    ...
    return out
st.set_page_config(page_title='Backtest', page_icon='ÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â°ÃƒÆ’Ã¢â‚¬Â¦Ãƒâ€šÃ‚Â¸ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬Ãƒâ€¦Ã¢â‚¬Å“ÃƒÆ’Ã¢â‚¬Â¹ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â\xa0', layout='wide')
st.header('Backtest')
...
market_sel = c4.multiselect('Markets (choose one or more)', options=list(_MARKET_GROUPS.keys()), default=['Moneyline', 'Spread', 'Total (O/U)', 'Props', 'Parlay'])
week_opts = list(range(1, 23)) if season_choice != 'All' else []
week_choice = c5.multiselect('Weeks (if season selected)', options=week_opts, default=week_opts, key=f'weeks_{season_choice}' if season_choice != 'All' else 'weeks_all')
    ...
    try:
        ...
        if base.empty:
            ...
            if week_choice and 'week' in base.columns:
                ...
                mask = wk.isin(week_ints) | wk.isna()
                filtered = base[mask]
                if len(filtered) > 0:
                    base = filtered
                else:
            ...
            if base.empty:
                ...
                if not curve.empty:
                    st.line_chart(curve.set_index('placed_at')['bankroll'], width='stretch')
                else:
                    st.caption('No settled bets to plot.')
                st.subheader('Bets (filtered)')
                show_cols = [c for c in ['placed_at', 'sport', 'league', 'game_id', 'season', 'week', 'market', 'market_group', 'ref', 'side', 'line', 'odds', 'p_win', 'ev', 'stake', 'stake_sim', 'result', 'pnl', 'policy', 'tag', 'src'] if c in pol_df.columns]
                st.dataframe(pol_df[show_cols] if show_cols else pol_df, width='stretch')
